...
use platform::{CapabilityLevel, LinkState};
    ...
    let listener = match listener_options.create_tokio() {
        Err(e) if e.kind() == std::io::ErrorKind::AddrInUse => {
            anyhow::bail!(
                ...
                protocol::SOCKET_NAME
            );
        }
        result => result?,
    };

    tracing::info!(socket = protocol::SOCKET_NAME, "listening for clients");
...
async fn supervise(service: Arc<SherdService>, interval: Duration) {
    ...
    loop {
        ...
        let is_up = |link: &Option<platform::LinkStatus>| {
