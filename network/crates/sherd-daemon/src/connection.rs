...
use sherd_core::{FeatureNotReady, SherdService};
use sherd_protocol::{decode_line, encode_line, Request, Response, ServerMessage};
...
async fn run_event_forwarder(
    mut events: tokio::sync::broadcast::Receiver<sherd_protocol::Event>,
...
fn unit_ok_or_error(result: sherd_platform::PlatformResult<()>) -> Response {
    match result {
        Ok(()) => Response::Ok,
...
fn ok_or_error<T>(
    result: sherd_platform::PlatformResult<T>,
