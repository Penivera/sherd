...
use desktop::auth::AuthManager;
use desktop::state::{AppScreen, AppState, AuthMethodInfo, MeshTask};
use desktop::theme::Theme;
use desktop::views::{render_auth, render_mesh, render_splash, render_toggle, ActiveField, AuthMode};
...
impl SherdApp {
    ...
    fn handle_key_down(&mut self, event: &KeyDownEvent, cx: &mut Context<Self>) {
        ...
        if (event.keystroke.modifiers.control && key.eq_ignore_ascii_case("d"))
            || key.eq_ignore_ascii_case("f2")
        {
...
impl Render for SherdApp {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .child(
                // Theme toggle button (fixed top right, on top of screen content)
                div()
                    .id("theme-toggle-btn")
                    .absolute()
                    .top(px(10.0))
